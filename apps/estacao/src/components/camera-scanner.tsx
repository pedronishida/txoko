      ...
      <video
        ...
        className={
          preview
            ...
            : 'fixed w-[120px] h-[90px] pointer-events-none'
        }
        style={
          preview
            ? undefined
...
function CameraStatusDot({ status, errorMsg }: { status: CameraStatus; errorMsg: string | null }) {
  // Quando ativa: bolinha verde minuscula no canto superior esquerdo (nao atrapalha).
  // Problema: pill completo com mensagem.
  if (status === 'active') {
    return (
      <div
        className="fixed top-4 left-4 z-40 w-2 h-2 rounded-full bg-primary"
        title="Camera ativa"
      />
    )
  }
