  ...
  return (
    <div className="space-y-6">
      ...
      {(!hasAvontade || !hasPorKg) && (
        <div className="rounded-xl border border-accent/30 bg-accent/10 p-4 flex gap-3">
          <AlertTriangle size={18} className="text-accent-foreground shrink-0 mt-0.5" />
          <div className="text-sm text-foreground space-y-1">
            <p className="font-semibold">Produtos self-service nao cadastrados</p>
            <p className="text-foreground/75">
              Antes de usar a estacao, cadastre no <Link href="/cardapio" className="underline hover:text-foreground">Cardapio</Link>:
            </p>
            <ul className="text-foreground/75 text-xs mt-1 space-y-0.5">
      ...
      {/* List */}
      {filtered.length === 0 ? (
        <div className="rounded-xl border border-border bg-night p-10 text-center">
        ...
        <div className="rounded-xl border border-border overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-night text-foreground/75 text-xs uppercase tracking-wide">
              <tr>
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h3 className="font-semibold text-foreground">Cartoes de cancelamento (caixa)</h3>
            ...
            {remaining > 0 && (
              <button
                onClick={onGenerate}
                className="shrink-0 inline-flex items-center gap-1.5 h-8 px-3 bg-destructive/20 text-destructive rounded-lg text-xs font-semibold hover:bg-destructive/30"
              >
                <Plus size={13} />
                Gerar
              </button>
          ...
          {active.length === 0 ? (
            <p className="mt-3 text-xs text-muted">Nenhum cartao de cancelamento ativo.</p>
          ) : (
            <div className="mt-3 flex flex-wrap gap-2">
              {active.map((c) => (
                <div
                  key={c.id}
                  className="inline-flex items-center gap-2 px-2.5 h-7 rounded-md bg-night border border-border text-xs"
                >
                  <span className="font-data text-foreground">#{String(c.card_number).padStart(3, '0')}</span>
                  <span className="text-muted font-data">{c.qr_token.slice(0, 8)}...</span>
              ...
              <Link
                ...
                target="_blank"
                className="inline-flex items-center gap-1.5 h-7 px-2.5 border border-border rounded-md text-xs text-foreground/75 hover:text-foreground hover:bg-muted-subtle"
              >
                <Printer size={11} />
                Imprimir
              </Link>
...
function CardToggleMini({ card }: { card: ComandaCard }) {
  ...
  return (
    <button
      ...
      disabled={pending}
      className="text-muted hover:text-destructive text-[10px]"
      title="Desativar"
    >
      {pending ? '...' : 'desativar'}
    </button>
...
function CancelBatchModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-surface border border-destructive/30 rounded-xl w-full max-w-md">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <div className="flex items-center gap-2">
            <Ban size={16} className="text-destructive" />
            <h2 className="font-semibold text-foreground">Gerar cartao de cancelamento</h2>
          </div>
        ...
        <form onSubmit={submit} className="p-5 space-y-4">
          <div>
            <label className="block text-sm font-medium text-foreground/75 mb-1">Quantidade</label>
          ...
          <div className="flex gap-3 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-2.5 border border-border rounded-lg text-sm font-medium text-foreground/75 hover:text-foreground hover:bg-muted-subtle"
            >
              Fechar
            </button>
            <button
              type="submit"
              disabled={pending}
              className="flex-1 py-2.5 bg-destructive text-white font-semibold rounded-lg text-sm hover:bg-destructive/80 disabled:opacity-50"
            >
...
function StatCard({
  ...
  return (
    <div className="rounded-xl border border-border bg-night p-4">
      ...
      <div className="mt-2 text-2xl font-data font-semibold text-foreground">{value}</div>
      {sub && <div className="text-foreground/75 text-xs mt-0.5">{sub}</div>}
    </div>
  )
}
  ...
  return (
    <tr className="text-foreground">
      ...
      <td className="px-4 py-2.5">
        <span
          className={
            'text-xs ' + (card.is_active ? 'text-success' : 'text-muted')
          }
        >
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-surface border border-border rounded-xl w-full max-w-md">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <h2 className="font-semibold text-foreground">Gerar lote de cartoes</h2>
          <button onClick={onClose} className="p-1.5 rounded-md text-muted hover:text-foreground hover:bg-muted-subtle">
            <X size={18} />
          </button>
        </div>
        ...
        <form onSubmit={submit} className="p-5 space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 bg-night border border-border rounded-lg text-sm text-foreground font-data focus:outline-none focus:ring-1 focus:ring-primary/30"
              required
            />
