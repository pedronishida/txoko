        ...
        <div className="px-4 py-2 sticky top-[113px] z-20 bg-night/95 backdrop-blur-md">
          <div className="flex items-center gap-2 overflow-x-auto pb-1 -mx-1 px-1 scrollbar-hide">
            {activeCategories.filter(c => productsByCategory[c.id]).map(cat => (
              <button key={cat.id} onClick={() => scrollToCategory(cat.id)} className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-colors ${selectedCategory === cat.id ? 'bg-leaf text-night' : 'bg-night-light text-stone-light border border-night-lighter'}`}>{cat.name}</button>
            ))}
          </div>
        </div>
      )}
