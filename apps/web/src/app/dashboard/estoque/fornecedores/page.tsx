...
import type { Supplier } from '@txoko/shared'
import { Plus, Phone, Mail, FileText, X, Pencil, Package } from 'lucide-react'

export default function FornecedoresPage() {
  const { suppliers, ingredients, addSupplier, updateSupplier } = useStore()
  ...
  const supplierIngredients = selected ? ingredients.filter(i => i.supplier_id === selected.id) : []

  function openForm(supplier: Supplier | null) {
    if (supplier) {
      setEditing(supplier)
      setFormName(supplier.name); setFormDocument(supplier.document || ''); setFormPhone(supplier.phone || ''); setFormEmail(supplier.email || ''); setFormNotes(supplier.notes || '')
    } else {
      setEditing(null)
      setFormName(''); setFormDocument(''); setFormPhone(''); setFormEmail(''); setFormNotes('')
    }
    setShowForm(true)
  }
  ...
  function handleSave() {
    if (!formName) return
    const data = { name: formName, document: formDocument || null, phone: formPhone || null, email: formEmail || null, address: null, notes: formNotes || null }
            ...
            {suppliers.map(sup => {
              ...
              return (
                <button
                  ...
                  className={`bg-night-light border rounded-xl p-4 text-left transition-colors ${selectedId === sup.id ? 'border-leaf/30' : 'border-night-lighter hover:border-night-lighter/80'}`}
                >
                  <div className="flex items-start justify-between">
                    <div>
                      <h3 className="font-semibold text-cloud">{sup.name}</h3>
                      {sup.document && <p className="text-xs text-stone font-data mt-0.5">{sup.document}</p>}
                    </div>
                    <button onClick={(e) => { e.stopPropagation(); openForm(sup) }} className="p-1 text-stone hover:text-cloud">
                      <Pencil size={14} />
                    </button>
                  </div>
                  <div className="flex items-center gap-4 mt-3 text-xs text-stone">
                    {sup.phone && <span className="flex items-center gap-1"><Phone size={12} />{sup.phone}</span>}
                    {sup.email && <span className="flex items-center gap-1"><Mail size={12} />{sup.email}</span>}
                  </div>
                  {sup.notes && <p className="text-xs text-stone-light mt-2 line-clamp-1">{sup.notes}</p>}
        ...
        {selected && (
          <div className="w-72 bg-night-light border border-night-lighter rounded-xl shrink-0">
            ...
            <div className="p-4 space-y-3">
              {selected.document && <div><p className="text-[10px] text-stone">CNPJ</p><p className="text-xs text-cloud font-data">{selected.document}</p></div>}
              {selected.phone && <div><p className="text-[10px] text-stone">Telefone</p><p className="text-xs text-cloud">{selected.phone}</p></div>}
              {selected.email && <div><p className="text-[10px] text-stone">Email</p><p className="text-xs text-cloud">{selected.email}</p></div>}
              {selected.notes && <div><p className="text-[10px] text-stone">Observacoes</p><p className="text-xs text-stone-light">{selected.notes}</p></div>}
              <div>
                <p className="text-[10px] text-stone mb-2">Insumos fornecidos</p>
                {supplierIngredients.length > 0 ? (
                  <div className="space-y-1">
                    {supplierIngredients.map(ing => (
                      <div key={ing.id} className="flex items-center justify-between text-xs bg-night rounded-lg px-2 py-1.5">
                        <span className="text-cloud">{ing.name}</span>
                        <span className="text-stone font-data">{ing.current_stock} {ing.unit}</span>
                      </div>
                    ))}
                  </div>
                ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-night-light border border-night-lighter rounded-xl w-full max-w-md">
            <div className="flex items-center justify-between px-5 py-4 border-b border-night-lighter">
              <h2 className="font-semibold text-cloud">{editing ? 'Editar Fornecedor' : 'Novo Fornecedor'}</h2>
            ...
            <div className="p-5 space-y-3">
              ...
              <div>
                ...
                <input value={formDocument} onChange={e => setFormDocument(e.target.value)} placeholder="00.000.000/0000-00" className="w-full px-3 py-2 bg-night border border-night-lighter rounded-lg text-sm text-cloud font-data focus:outline-none focus:ring-1 focus:ring-leaf/50" />
              </div>
              ...
              <div>
                ...
                <textarea value={formNotes} onChange={e => setFormNotes(e.target.value)} rows={2} className="w-full px-3 py-2 bg-night border border-night-lighter rounded-lg text-sm text-cloud focus:outline-none focus:ring-1 focus:ring-leaf/50 resize-none" />
              </div>
