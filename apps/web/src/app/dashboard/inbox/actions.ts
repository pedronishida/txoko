...
export async function sendMessage(input: {
  ...
  // Carrega conversa + canal + destinatario externo para decidir envio
  const { data: conv, error: convErr } = await supabase
    .from('conversations')
    .select(
      `id, channel_id, contact_id,
       channel:channels(id, type, config, status),
       identities:contact_identities!inner(external_id, channel_id)`
    )
  ...
  const identities = (conv.identities ?? []) as Array<{
    external_id: string
    channel_id: string
  }>
  const identity = identities.find((i) => i.channel_id === conv.channel_id)
...
export async function markConversationRead(conversationId: string) {
  ...
  const channel = conv?.channel
    ...
    : null
  const identity = conv
    ? ((conv.identities ?? []) as Array<{ external_id: string; channel_id: string }>).find(
        (i) => i.channel_id === conv.channel_id
      )
    : null
