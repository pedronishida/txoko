...
import { X, Users, Clock, UtensilsCrossed, QrCode } from 'lucide-react'

const STATUS_CONFIG: Record<TableStatus, { label: string; color: string; bg: string }> = {
  available: { label: 'Disponivel', color: 'text-leaf', bg: 'bg-leaf/10 border-leaf/30' },
...
export default function MesasPage() {
  const { tables, orders, orderItems, products, updateTableStatus, getTableOrder } = useStore()
          ...
          {areas.map(area => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium capitalize transition-colors ${
                filterArea === area ? 'bg-leaf/10 text-leaf' : 'bg-night-light text-stone-light hover:text-cloud border border-night-lighter'
              }`}
            >
      ...
      <div className="flex items-center gap-4">
        {Object.entries(STATUS_CONFIG).map(([status, config]) => (
          <div key={status} className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-stone">{config.label} ({counts[status as TableStatus]})</span>
          </div>
        ))}
      </div>
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    config.bg,
                    selectedTable?.id === table.id && 'ring-2 ring-leaf ring-offset-2 ring-offset-night',
                    'hover:scale-[1.02]'
                  )}
                >
                  ...
                  {table.status === 'occupied' && minutes > 0 && (
                    <div className={cn(
                      ...
                      minutes > 60 ? 'bg-coral text-white' : minutes > 30 ? 'bg-warm text-night' : 'bg-night-lighter text-cloud'
                    )}>
                      {minutes}m
                    </div>
                  )}
                  {order && (
                    <p className="text-[10px] text-stone font-data mt-1">{formatCurrency(order.total)}</p>
                  )}
                </button>
              )
            })}
          ...
          <div className="w-80 bg-night-light border border-night-lighter rounded-xl flex flex-col">
            <div className="px-4 py-3 border-b border-night-lighter flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-stone">{selectedTable.capacity} lugares — {selectedTable.area === 'main' ? 'Salao' : 'Terraco'}</p>
              </div>
            ...
            <div className="px-4 py-2 border-b border-night-lighter">
              ...
              {showQR && (
                <div className="mt-2 p-3 bg-white rounded-lg flex flex-col items-center">
                  {/* Simple QR Code via external API */}
                  <img
                    src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(`${typeof window !== 'undefined' ? window.location.origin : ''}/menu/txoko-demo?mesa=${selectedTable.number}`)}`}
                    alt={`QR Code Mesa ${selectedTable.number}`}
                    width={150}
                    height={150}
                    className="rounded"
                  />
                  <p className="text-xs text-night font-medium mt-2">Mesa {selectedTable.number}</p>
                  <p className="text-[10px] text-stone-dark mt-0.5">/menu/txoko-demo?mesa={selectedTable.number}</p>
                </div>
              )}
            </div>
              ...
              <div className="flex-1 overflow-y-auto">
                <div className="px-4 py-3 border-b border-night-lighter">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-data text-leaf font-medium">#{selectedOrder.id.split('-')[1]}</span>
                  </div>
                  <div className="flex items-center justify-between mt-1">
                    ...
                    <span className="text-xs font-data text-cloud flex items-center gap-1">
                      <Clock size={12} />
                      {getOccupiedMinutes(selectedOrder.created_at)}min
                    </span>
                  </div>
                </div>
                  ...
                  {selectedItems.map(item => {
                    ...
                    return (
                      <div key={item.id} className="px-4 py-2 flex items-center justify-between">
                        <div>
                          <p className="text-sm text-cloud">{item.quantity}x {product?.name}</p>
                          {item.notes && <p className="text-[10px] text-warm">{item.notes}</p>}
                        </div>
                        <span className="text-xs font-data text-stone">{formatCurrency(item.total_price)}</span>
                      </div>
                    )
                  })}
                </div>
